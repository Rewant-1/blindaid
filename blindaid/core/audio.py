...
class GTTSAudioPlayer:
    """Audio player using Google TTS (requires internet)."""

    def __init__(self):
        self.queue: Queue[str | None] = Queue(maxsize=5)
        self.worker_thread = Thread(target=self._worker, daemon=True)
        self.worker_thread.start()
        logger.info("Audio player initialized with gTTS")
    ...
    def _worker(self):
        ...
        try:
            ...
            logger.error("gTTS/pygame not available - audio disabled: %s", exc)
