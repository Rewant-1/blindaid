...
\author{
\IEEEauthorblockN{Rewant Bhriguvanshi}
\IEEEauthorblockA{
\textit{Department of ECE}\\
\textit{Faculty of Technology}\\
\textit{University of Delhi}\\
Delhi, India\\
rewant.bhriguvanshi1@gmail.com
}
\and
\IEEEauthorblockN{Shubhika Sinha}
\IEEEauthorblockA{
\textit{Department of CSE}\\
\textit{Faculty of Technology}\\
\textit{University of Delhi}\\
Delhi, India\\
dr.asinha@ce.du.ac.in
}
\and
...
devices---i.e., autonomous robots, IoT monitoring platforms, mobile
...
At 10\,fps (100\,ms per frame), scene conditions change slowly
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lccc@{}}
    ...
    Motion-Triggered & $\infty$  & 69.6  & $\infty$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
$r = 0.876$, this gives $\bar{t} \approx 10.8$\,ms which is well within
real-time requirements at 10\,fps (100\,ms budget).
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lccc@{}}
    ...
    Static 1/5        & 90.5 & 79.8 & 4.48 \\
    Static 1/10       & 78.0 & 89.7 & 7.60 \\
    Static 1/15       & 56.4 & 93.1 & 8.16 \\
    \midrule
    Random Skip       & 78.1 & 82.6 & 4.50 \\
    Optical Flow      & 96.3 & 66.8 & 2.90 \\
    Motion-Triggered  & 93.9 & 69.6 & 3.09 \\
    \midrule
    AFP Stab-only     & 61.5 & 92.0 & 7.67 \\
    AFP Prox-only     & 81.4 & 86.2 & 5.92 \\
...
93.9\%) but process too many frames (skip ratios of only 66.8\% and
69.6\%), yielding low CEI values of 2.90 and 3.09. Walking motion
...
baselines. Clip~12 (high-density, average proximity~$= 0.78$)
...
\textbf{Limitations.} AFP uses manually-tuned parameters rather than a
learned policy, but the main novelty is not the exact threshold
values ($\tau_p$, $s_\text{min}$, $s_\text{max}$), but the scheduling
formulation itself---the depth feedback architecture which breaks
the circular dependency, the formal latency bounds which follow from
it, and that a simple heuristic instantiation of this formulation
already meets static baselines. A learned policy could be substituted
within the same formulation to discover better strategies in more
complex environments. The system only relies on monocular relative
depth (MiDaS) which gives approximate rather than metric distance
estimates; because MiDaS outputs relative depth normalized to $[0,1]$,
proximity threshold $\tau_p = 0.7$ does not correspond to a fixed
physical distance and may need recalibration across scene types.
Indoor corridors with uniform depth gradients have different
normalizations than open outdoor environments. However, the parameter
sensitivity analysis (Table~\ref{tab:sensitivity}) suggests AFP
degrades gracefully across a wide $\tau_p$ range (0.5--0.9), giving a
practical tuning envelope for deployment. A practical mitigation is a
brief per-deployment calibration pass: on start-up, run AFP in
Always-On mode for $W$ warmup frames (e.g.\ $W{=}100$, $\sim$10\,s at
10\,fps), collect the resulting distribution of center-region depth
values $d_t$, and set $\tau_p$ to a fixed percentile (e.g.\ the 85th)
of that distribution rather than a hardcoded constant. This will
adapt the threshold to each scene's depth normalization
automatically---an indoor corridor and an open plaza would each
converge to a different absolute $\tau_p$ but the same relative
proximity criterion---at the cost of a one-time warmup delay before
AFP's latency bound becomes active. While AFP Full reaches 77.9\%
average coverage, approximately 22\% of detections are missed,
concentrated during stable or far-field periods. Under close
proximity, AFP ramps up sampling frequency to reduce gaps. The
evaluation was run on a single hardware platform; to strengthen
generalizability, benchmarking across embedded targets is a worthy
pursuit. While direct hardware validation is left to future work,
published ONNX/INT8 benchmarks for comparably-sized detectors give a
rough envelope: YOLOv8n-class models report per-frame latencies of
roughly 20--80\,ms on Raspberry Pi 5 and Jetson Orin-class hardware~\cite{pi5adas2026,jetsonyolo2025}, versus 30.0\,ms on our x86 test platform. Adding a
similarly-scaled MiDaS-Small pass, we would expect combined per-frame
latency in the 100--250\,ms range on such devices---well within AFP's
tolerance, since the framework bounds worst-case latency independent
of raw per-frame cost by simply reducing $s_\text{min}$ to compensate
for a slower platform. The relative gains over static subsampling
would persist, though the absolute latency bound
(Eq.~\ref{eq:afp_latency}) would shift proportionally with the
platform's frame rate $f$.
