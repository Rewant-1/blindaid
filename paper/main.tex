...
1/10 subsampling ($p = 0.79$, Wilcoxon signed-rank), while
bounding worst-case proximity response latency at 100\,ms compared
...
\begin{enumerate}
  ...
  \item A formal analysis of worst-case response latency under
    ...
    AFP bounds proximity latency at 100\,ms while static 1/10
  ...
  \item An empirical evaluation on 24 real-world video sequences
    ...
    subsampling ($p = 0.79$, Wilcoxon signed-rank test).
\end{enumerate}
...
$L_\text{AFP}^\text{worst} = 100$\,ms---a $9\times$ improvement
...
travel before the system reacts, while AFP limits this to 0.12\,m.
...
at 100\,ms. A Wilcoxon signed-rank test across the 24 sequences
confirms no statistically significant difference in coverage
($p = 0.79$). The average response delay drops from 2.4~frames
(static) to 2.1~frames (AFP).
...
Across all 24 sequences, a Wilcoxon signed-rank test confirms no
statistically significant difference between AFP and Static~1/10
coverage ($p = 0.79$). The average response delay is 2.1~frames
(AFP) versus 2.4~frames (Static~1/10), with AFP's worst-case
proximity delay bounded at 1~frame (100\,ms).
...
approximate rather than metric distance estimates. While AFP Full
...
and bounding worst-case proximity latency at 100\,ms---a
$9\times$ reduction from the 900\,ms incurred by static~1/10.
